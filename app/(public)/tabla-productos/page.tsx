...
export default async function TablaProductosPage() {
  const categories = await prisma.category.findMany({
    where: { active: true, products: { some: { active: true, status: 'PUBLISHED' } } },
    include: {
      products: {
      ...
      _count: { select: { products: { where: { active: true, status: 'PUBLISHED' } } } },
    },
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-10 gap-4">
          ...
          <div className="bg-secondary/50 backdrop-blur-md border border-white/10 px-4 py-3 rounded-lg inline-flex items-center gap-2">
            ...
            <span className="text-white font-bold">{totalProducts}</span> <span className="text-text-secondary">productos en total</span>
          </div>
        </div>

        <div className="space-y-6">
          {categories.map((category) => (
            <section key={category.id} className="bg-card/40 backdrop-blur-xl border border-white/10 rounded-lg overflow-hidden shadow-2xl">
              <div className="flex flex-col gap-1 border-b border-white/10 bg-secondary/55 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                ...
                <span className="w-fit rounded-lg border border-accent/20 bg-accent/10 px-3 py-1 text-sm font-bold text-accent">
                  ...
                  return (
                    <Link key={product.id} href={`/producto/${product.slug}`} className="flex gap-3 p-3 transition-colors hover:bg-white/[0.03]">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-xs text-text-secondary">{product.shortDescription || 'Sin descripción'}</p>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-secondary/80 border-b border-white/10 text-text-secondary text-sm uppercase tracking-wider">
                      ...
                      <th className="p-4 font-semibold">Marca / variante</th>
                        ...
                        <tr key={product.id} className="hover:bg-white/[0.02] transition-colors group">
                          ...
                          <td className="p-4">
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-accent/10 text-accent border border-accent/20">
                              {product.brand || product.sizes || product.colors || 'General'}
                            </span>
                          </td>
                          <td className="p-4">
